    ...
    <div class="expense-categories-container">
        <div class="page-header">
            ...
            <div class="action-bar">
                <button @click="showModal = true" class="btn btn-primary">Add Expense Category</button>
                <button class="btn outline" @click="showImport = true">Import Expense Categories</button>
            </div>
        </div>
        <div class="filters">
        ...
        <div ref="tableContainer" class="table-container" @scroll="handleScroll">
            <table class="data-table">
                ...
                <tbody>
                    <tr v-for="(cat, idx) in filteredCategories" :key="cat?.id ?? `ec-${idx}`">
                        <td>{{ cat.name }}</td>
                        <td>{{ cat.code || 'N/A' }}</td>
                        <td>{{ cat.description || 'N/A' }}</td>
                        <td>{{ cat.expenses_count ?? 0 }}</td>
                        <td>
                            <span :class="cat.is_active ? 'badge-success' : 'badge-danger'">
                                {{ cat.is_active ? 'Active' : 'Inactive' }}
                            </span>
                        </td>
                        <td>
                            <button @click="editCategory(cat)" class="btn-sm btn-primary">Edit</button>
                            <button @click="deleteCategory(cat.id)" class="btn-sm btn-danger">Delete</button>
                        </td>
                    </tr>
                </tbody>
            </table>
            ...
            <div v-if="!hasMore && filteredCategories.length > 0" class="no-more-indicator">No more categories</div>
        </div>
        ...
        <div v-if="showModal" class="modal-overlay" @click="showModal = false">
            <div class="modal-content" @click.stop>
                ...
                <form @submit.prevent="saveCategory">
                    ...
                    <div class="form-group">
                        <label>Code</label>
                        <input v-model="form.code" type="text" placeholder="Optional code" />
                    </div>
                    <div class="form-group">
        ...
        <ImportModal
            ...
            @close="showImport = false; loadInitial()"
        />
    </div>
</template>
...
export default {
    ...
    setup() {
        ...
        const {
            ...
            updateFilter
        } = usePaginatedDropdown('/api/expense-categories', {
            searchParam: 'search',
            initialFilters: {},
            perPage: 10
        });
        ...
        const saveCategory = async () => {
            try {
                if (editingCategory.value) {
                    await axios.put(`/api/expense-categories/${editingCategory.value.id}`, form.value);
                } else {
...
<style scoped>
.expense-categories-container { padding: 20px; }
.page-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; }
.action-bar { display: flex; gap: 12px; }
.filters { display: flex; gap: 15px; margin-bottom: 20px; }
.search-input, .select-input { padding: 10px; border: 1px solid #ddd; border-radius: 8px; flex: 1; }
